...
using dec_data = DecodeState;
using EventEmitter = std::function<void(events::Variant const&)>;
...
namespace
{
    ...
    struct ModeA
    {
        ...
        inline static constexpr auto  NCOSTAS  = protocol::CostasType::Original;
    ...
    struct ModeB
    {
        ...
        inline static constexpr auto  NCOSTAS  = protocol::CostasType::Modified;
...
namespace
{
    ...
    constexpr int BP_MAX_ITERATIONS = 60; // Max iterations in BP decoder (increased for Android)
    ...
    {
#ifdef __ANDROID__
        // Check for NaN/Inf in input LLRs
        int nan_count = 0;
        int inf_count = 0;
        for (const auto& val : llr) {
            if (std::isnan(val)) nan_count++;
            if (std::isinf(val)) inf_count++;
        }
        if (nan_count > 0 || inf_count > 0) {
            __android_log_print(ANDROID_LOG_ERROR, "JS8Decoder",
                               "bpdecode174: Input LLR has %d NaN, %d Inf values", nan_count, inf_count);
        }
#endif
...
namespace
{
    ...
    class DecodeMode
    {
        ...
        inline static const auto& Costas = protocol::costas(Mode::NCOSTAS);
               ...
               EventEmitter emitEvent)
        {
...
#ifdef __ANDROID__
            static int xdt_log_count = 0;
            if (xdt_log_count++ < 3) {
                __android_log_print(ANDROID_LOG_INFO, "JS8Decoder",
                    "xdt→i0 calc: xdt=%.3f, ASTART=%.3f, FS2=%.1f, i0=%d, search_range=[%d,%d]",
                    xdt, Mode::ASTART, FS2, i0, i0 - Mode::NQSYMBOL, i0 + Mode::NQSYMBOL);
            }
#endif
...
#ifdef __ANDROID__
            static int ibest_log_count = 0;
            if (ibest_log_count++ < 3) {
                __android_log_print(ANDROID_LOG_INFO, "JS8Decoder",
                    "Symbol extraction: ibest=%d, Mode::NDFFT2=%d, Mode::NDOWNSPS=%d, NN=%d",
                    ibest, Mode::NDFFT2, Mode::NDOWNSPS, NN);
                __android_log_print(ANDROID_LOG_INFO, "JS8Decoder",
                    "Symbol range: [%d, %d], cd0 has %d samples after IFFT",
                    ibest, ibest + (NN-1) * Mode::NDOWNSPS + Mode::NDOWNSPS, Mode::NDFFT2);
            }
#endif
...
#ifdef __ANDROID__
                else if (ibest_log_count <= 3 && k < 3) {
                    __android_log_print(ANDROID_LOG_INFO, "JS8Decoder",
                        "Symbol %d SKIPPED: i1=%d out of range [0, %d]", k, i1, NP2 - Mode::NDOWNSPS);
                }
#endif
...
#ifdef __ANDROID__
                // Log first few symbols to verify tone discrimination
                static int symbol_log_count = 0;
                if (symbol_log_count < 3 && k < 5) {
                    __android_log_print(ANDROID_LOG_INFO, "JS8Decoder",
                        "Symbol %d powers (bins 0-7): %.2e %.2e %.2e %.2e %.2e %.2e %.2e %.2e",
                        k, s2[0][k], s2[1][k], s2[2][k], s2[3][k],
                        s2[4][k], s2[5][k], s2[6][k], s2[7][k]);
                    if (k == 4) symbol_log_count++;
                }
#endif
            }
            ...
            if (syncStats)
            {
                events::SyncState evt;
                evt.kind = events::SyncState::Kind::Candidate;
                evt.mode = Mode::NSUBMODE;
                evt.frequency = f1;
                evt.dt = xdt;
                evt.sync.candidate = nsync;
                emitEvent(evt);
            }
            ...
            for (int ipass = 1; ipass <= 4; ++ipass)
            {
                    ...
                    !(ipass     == 4    && nharderrors > 30))
                {
                   ...
                   if (crc_ok)
                   {
                        ...
                        {
                            events::SyncState evt;
                            evt.kind = events::SyncState::Kind::Decoded;
                            evt.mode = Mode::NSUBMODE;
                            evt.frequency = f1;
                            evt.dt = xdt2;
                            evt.sync.decoded = sync;
                            emitEvent(evt);
                        }
                        ...
                        legacy_encode(i3bit, Costas, message.data(), itone.data());
        ...
        js8_downsample(float const f0)
        {
            ...
            int const rotation_amount = i0 - ib;

#ifdef __ANDROID__
            static int rotate_log_count = 0;
            if (rotate_log_count++ < 3) {
                __android_log_print(ANDROID_LOG_INFO, "JS8Decoder",
                    "Rotation params: f0=%.1f, i0=%d, ib=%d, it=%d, rotation=%d, RANGE_SIZE=%zu",
                    f0, i0, ib, it, rotation_amount, RANGE_SIZE);
                __android_log_print(ANDROID_LOG_INFO, "JS8Decoder",
                    "Before rotation - first 8 bins:");
                for (int i = 0; i < 8; i += 4) {
                    __android_log_print(ANDROID_LOG_INFO, "JS8Decoder",
                        "  Bins %d-%d: %.2e %.2e %.2e %.2e",
                        i, i+3,
                        std::abs(cd0[i]), std::abs(cd0[i+1]),
                        std::abs(cd0[i+2]), std::abs(cd0[i+3]));
                }
            }
#endif

            std::rotate(cd0.begin(), cd0.begin() + rotation_amount, cd0.begin() + Mode::NDFFT2);

#ifdef __ANDROID__
            if (rotate_log_count <= 3) {
                __android_log_print(ANDROID_LOG_INFO, "JS8Decoder",
                    "Before inverse FFT - cd0[0-7]: %.2e %.2e %.2e %.2e %.2e %.2e %.2e %.2e",
                    std::abs(cd0[0]), std::abs(cd0[1]), std::abs(cd0[2]), std::abs(cd0[3]),
                    std::abs(cd0[4]), std::abs(cd0[5]), std::abs(cd0[6]), std::abs(cd0[7]));
                __android_log_print(ANDROID_LOG_INFO, "JS8Decoder",
                    "RANGE_SIZE=%zu, NDFFT2=%d, after rotation cd0 has data", RANGE_SIZE, Mode::NDFFT2);
            }
#endif
            ...
            if (rotate_log_count <= 3) {
                __android_log_print(ANDROID_LOG_INFO, "JS8Decoder",
                    "After inverse FFT - cd0[0-7]: %.2e %.2e %.2e %.2e %.2e %.2e %.2e %.2e",
                    std::abs(cd0[0]), std::abs(cd0[1]), std::abs(cd0[2]), std::abs(cd0[3]),
                    std::abs(cd0[4]), std::abs(cd0[5]), std::abs(cd0[6]), std::abs(cd0[7]));
            }
#endif
    ...
    public:
                   ...
                   EventEmitter     emitEvent)
        {
            ...
            if (data.params.syncStats) emitEvent(events::SyncStart{pos, sz});
            ...
            if ((JS8_RX_SAMPLE_SIZE - pos) < sz)
            {
                ...
                ddCopy(std::begin(data.samples) + pos, std::begin(data.samples) + pos + firstsize,  dd.begin());
                ddCopy(std::begin(data.samples),       std::begin(data.samples) +       secondsize, dd.begin() + firstsize);
            }
            else
            {
                // Non-wrapping case; copy directly.

                ddCopy(std::begin(data.samples) + pos, std::begin(data.samples) + pos + sz, dd.begin());
            }
                ...
                for (auto [f1, xdt, sync] : candidates)
                {
                                             ...
                                             emitEvent))
                    {
                                      ...
                                      inserted || it->second < snr)
                        {
                            ...
                            emitEvent(events::Decoded{data.params.utc,
                                                      snr,
                                                      xdt - Mode::ASTART,
                                                      f1,
                                                      it->first.data,
...
#ifdef __ANDROID__
    // Debug: Log that we're about to emit DecodeStarted
    __android_log_print(ANDROID_LOG_INFO, "JS8Decoder",
                       "About to emit DecodeStarted, nsubmodes=0x%x, emit_fn=%s",
                       set, emit_fn ? "SET" : "NULL");
#endif

    emit(events::DecodeStarted{set});
...
#ifdef __ANDROID__
    __android_log_print(ANDROID_LOG_INFO, "JS8Decoder",
                       "DecodeFinished emitted, returning %zu", sum);
#endif
